...
function startGame() {
  ...
  if (isFirstRun()) {
    // Enter the first available encounter to teach mechanics
...
function enemyRespondToPlayer(pCard, pRow) {
  ...
  const hand = G.enemy.hand;
  if (hand.length === 0) {
    G.enemyCard = { suit: BATTLE_SUITS[Math.floor(Math.random()*4)], rank: 2, id: 'epass' };
    return;
  }
...
function playCard() {
  ...
  if (outcome.wins) {
    ...
    // FEATURE 2: Off-Suit Tactical Retreats
    if (!outcome.effectiveMatch) {
      // Off-suit on Foundation: DOUBLE shield
      if (row === 'foundation') {
        const extraShield = hasArchetypeMilestone('shield_boost') ? 8 : 3;
        G.shield += extraShield;
        spawnKeywordPopup('🛡️ TACTICAL RETREAT: +' + (extraShield * 2) + ' total shield!', 48);
      }
      // Off-suit on Heart: boost a random modifier
      if (row === 'heart' && G.modifiers.length > 0) {
        const eligible = G.modifiers.filter(m => m.type === 'mult');
        if (eligible.length > 0) {
          const target = eligible[Math.floor(Math.random() * eligible.length)];
          target.value = +(target.value + 0.05).toFixed(2);
          spawnKeywordPopup('❤️ RETREAT BOOST: ' + target.name + ' +0.05×', 48);
        } else {
          // Fallback: add a small chip mod
          addSparkModifier(null, 'chips');
          spawnKeywordPopup('❤️ RETREAT BOOST: +chip modifier!', 48);
        }
      }
    }
...
function showRewardScreen(inkGain, isBoss) {
  ...
  // FEATURE 4: Guaranteed keyword card matching lead archetype
  let guaranteedIndex = lead ? Math.floor(Math.random() * numRewards) : -1;
...
function renderPlayPreview() {
  ...
  if (outcome.wins) {
    ...
    // Feature 2: Off-suit tactical retreat bonuses
    if (G.selectedRow === 'foundation') {
      html += `<div class="preview-row"><span class="pr-label">Retreat Bonus</span><span class="pr-value shield">+${foundationShield} extra shield (double!)</span></div>`;
    }
    if (G.selectedRow === 'heart' && G.modifiers.length > 0) {
      html += `<div class="preview-row"><span class="pr-label">Retreat Bonus</span><span class="pr-value keyword">+0.05× random mod</span></div>`;
    }
...
function renderModifierStrip() {
  ...
  sorted.forEach((m, i) => {
    ...
    // Flash animation for newly added mods (last ones)
    if (isNew && globalIdx >= G.modifiers.length - 1) {
      pip.classList.add('mod-new');
    }
