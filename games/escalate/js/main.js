...
async function loadGameData() {
  ...
  // REC10v7: Keyword iconography — small emoji icons for each keyword
  window.KEYWORD_ICONS = {
    Swift: '⚡', Echo: '🔁', Phantom: '👻', Crown: '👑',
    Bloom: '🌸', Anchor: '⚓', Volatile: '💥', Absorb: '🛡️',
    Linked: '🔗', Fracture: '💔', Pyre: '🔥', Resurrect: '✝️',
    Burn: '🔥', Mirror: '🪞', Gambit: '🎲', Siphon: '🩸',
    Cascade: '🌊', Inscription: '📜', Curse: '🌑'
  };
...
// ============================================================
// REC1-v8: GUIDED FIRST-TRICK WALKTHROUGH
// Replaces static tutorial with inline guided play for first encounter.
// Trick 1: "Match the Suit" — highlights matching cards, dims others
// Trick 2: "Choose Your Row" — annotates row benefits (Stage 2+)
// Trick 3: "Watch the Engine" — annotates score breakdown
// ============================================================
const WALKTHROUGH_KEY = 'tee_walkthrough_done_v1';

function isWalkthroughEligible() {
  try {
    if (localStorage.getItem(WALKTHROUGH_KEY)) return false;
  } catch(e) { return false; }
  if (G._isDailyChallenge) return false;
  if (G.encountersWon > 0) return false;
  if (getOnboardingStage() > 2) return false; // Only for new-ish players
  return true;
}
...
function startGuidedWalkthrough() {
  if (!isWalkthroughEligible()) return;
  G._guidedWalkthrough = true;
  G._guidedTrickNum = 0;
}

function showWalkthroughGuidance() {
  if (!G._guidedWalkthrough) return;
  G._walkthroughShown = true;
  const trickNum = G._guidedTrickNum;
  
  // Remove any existing walkthrough overlay
  const existing = document.querySelector('.walkthrough-guidance');
  if (existing) existing.remove();
  
  const guidance = document.createElement('div');
  guidance.className = 'walkthrough-guidance walkthrough-step-' + trickNum;
  
  if (trickNum === 1) {
    // Trick 1: "Match the Suit"
    const enemySuit = G.enemyCard ? G.enemyCard.suit : null;
    const suitSymbol = enemySuit ? (SUIT_SYMBOLS[enemySuit] || '') : '';
    const suitColor = enemySuit ? getComputedSuitColor(enemySuit) : '#fff';
    
    if (enemySuit && !G.playerLeads) {
      guidance.innerHTML = `
        <div class="wt-arrow wt-arrow-down"></div>
        <div class="wt-message">
          <div class="wt-step-badge">Step 1 of 3</div>
          <div class="wt-title">Match the Suit!</div>
          <div class="wt-body">Play a <span style="color:${suitColor};font-weight:700">${suitSymbol} ${enemySuit}</span> card with a <strong>higher rank</strong> to win this trick.</div>
          <div class="wt-hint">Highlighted cards can win. Dimmed cards are off-suit.</div>
        </div>
      `;
      // Highlight matching cards in hand
      highlightMatchingCards(enemySuit);
    } else {
      guidance.innerHTML = `
        <div class="wt-arrow wt-arrow-down"></div>
        <div class="wt-message">
          <div class="wt-step-badge">Step 1 of 3</div>
          <div class="wt-title">You Lead!</div>
          <div class="wt-body">Pick any card — the enemy must try to match <strong>your</strong> suit. Higher rank wins!</div>
          <div class="wt-hint">Hearts boost multiplier. Diamonds boost chips.</div>
        </div>
      `;
    }
  } else if (trickNum === 2 && getOnboardingStage() >= 2) {
    // Trick 2: "Choose Your Row"
    guidance.innerHTML = `
      <div class="wt-message">
        <div class="wt-step-badge">Step 2 of 3</div>
        <div class="wt-title">Choose Your Row!</div>
        <div class="wt-body">Each row gives different benefits:</div>
        <div class="wt-row-hints">
          <div class="wt-row-hint" style="border-color:var(--crown-row)"><strong style="color:var(--crown-row)">Crown</strong> — ×Damage multiplier</div>
          <div class="wt-row-hint" style="border-color:var(--heart-row)"><strong style="color:var(--heart-row)">Heart</strong> — +Bonus chips</div>
          <div class="wt-row-hint" style="border-color:var(--foundation-row)"><strong style="color:var(--foundation-row)">Shield</strong> — Defense on loss</div>
        </div>
        <div class="wt-hint">Win 4 in a row on any lane to trigger a powerful Surge!</div>
      </div>
    `;
  } else if (trickNum === 2 && getOnboardingStage() < 2) {
    // Stage 1 only has Crown — skip row guidance, give encouragement
    guidance.innerHTML = `
      <div class="wt-message">
        <div class="wt-step-badge">Step 2 of 3</div>
        <div class="wt-title">Keep Going!</div>
        <div class="wt-body">Every win makes your next trick <strong>stronger</strong>. Watch the modifiers stack up!</div>
        <div class="wt-hint">Losing isn't all bad — it charges your Sacrifice counter!</div>
      </div>
    `;
  } else if (trickNum === 3) {
    // Trick 3: "Watch the Engine"
    guidance.innerHTML = `
      <div class="wt-message">
        <div class="wt-step-badge">Step 3 of 3</div>
        <div class="wt-title">Watch the Engine!</div>
        <div class="wt-body">When you win, watch the <strong>score breakdown</strong> — each modifier <strong>multiplies</strong> your damage. The more you win, the crazier the numbers get!</div>
        <div class="wt-hint">This is The Escalation Engine. You're ready!</div>
      </div>
    `;
  }
  
  // Insert guidance into battle screen
  const battleScreen = document.getElementById('battle-screen');
  if (battleScreen && guidance.innerHTML) {
    battleScreen.appendChild(guidance);
    requestAnimationFrame(() => guidance.classList.add('visible'));
    
    // Auto-dismiss after 8 seconds
    setTimeout(() => {
      if (guidance.parentElement) {
        guidance.classList.remove('visible');
        setTimeout(() => guidance.remove(), 400);
      }
    }, 8000);
  }
  
  // Mark done on trick 3
  if (trickNum >= 3) {
    G._guidedWalkthrough = false;
    markWalkthroughDone();
  }
}

function showPostTrickGuidance(won) {
  if (!G._guidedWalkthrough && G._guidedTrickNum < 3) return;
  
  const existing = document.querySelector('.walkthrough-post');
  if (existing) existing.remove();
  
  const trickNum = G._guidedTrickNum;
  const post = document.createElement('div');
  post.className = 'walkthrough-post';
  
  if (trickNum === 1 && won) {
    post.innerHTML = `<div class="wt-post-msg wt-post-win">✅ You matched the suit with a higher rank — that's how you win tricks!</div>`;
  } else if (trickNum === 1 && !won) {
    const reason = G._lastLossReason || 'The enemy had a higher rank or you played off-suit.';
    post.innerHTML = `<div class="wt-post-msg wt-post-loss">💡 ${reason}. Losses charge your <strong>Sacrifice</strong> counter — at 3 charges, your next win gets ×2.5!</div>`;
  }
  
  if (post.innerHTML) {
    const battleScreen = document.getElementById('battle-screen');
    if (battleScreen) {
      battleScreen.appendChild(post);
...
function highlightMatchingCards(enemySuit) {
  setTimeout(() => {
    document.querySelectorAll('.hand-card').forEach(el => {
      const cardSuit = el.dataset.suit;
      if (cardSuit === enemySuit || cardSuit === 'stars') {
        el.classList.add('wt-match');
      } else {
        el.classList.add('wt-dim');
      }
    });
  }, 100);
}
...
function clearWalkthroughHighlights() {
  document.querySelectorAll('.wt-match, .wt-dim').forEach(el => {
    el.classList.remove('wt-match', 'wt-dim');
  });
  const guidance = document.querySelector('.walkthrough-guidance');
  if (guidance) guidance.remove();
  const post = document.querySelector('.walkthrough-post');
  if (post) post.remove();
}
...
function initState(playerClass) {
  ...
  G = {
    ...
    // REC1-v8: Guided First-Trick Walkthrough state
    _guidedWalkthrough: false,  // Is walkthrough active this encounter?
    _guidedTrickNum: 0,         // Which guided trick we're on (1-3)
    _walkthroughShown: false,   // Has guidance been shown for current trick?
    // REC3-v8: Guaranteed Aha Moment state  
    _ahaTriggered: false,       // Has aha been triggered this run?
    _firstWinEver: false,       // Tracks first win in first encounter
    _enc1AllLosses: true,       // True until player wins in encounter 1
    // REC4-v8: Power Level tracking
    _prevPowerLevel: 0,         // For animated increment detection
  };
...
function isAhaMomentEligible() {
  // REC3-v8: Guaranteed Aha Moment — trigger on first win in first 2 encounters
  try {
    if (localStorage.getItem(AHA_KEY)) return false;
  } catch(e) { return false; }
  if (getOnboardingStage() > 2) return false; // Only for newer players
  if (G._isDailyChallenge) return false;
  if (G._ahaTriggered) return false;
  if (G.encountersWon > 1) return false; // Allow through encounter 2 as fallback
  return true;
}
...
function applySpectacleMultiplier(scoreResult) {
  // REC3-v8: Boost score to 100-150 range for maximum "wow" factor
  if (!G._spectacleTrickActive) return scoreResult;
  const targetScore = 100 + Math.floor(Math.random() * 51); // 100-150
...
function showAhaPopup(score, isVengeanceVariant) {
  G._spectacleTrickActive = false;
  markAhaDone();

  // REC3-v7: Escalation Preview — animated demo showing damage scaling
  const baseScore = Math.max(12, Math.floor(score / 8) || 12);
  ...
  // REC3-v8: Vengeance variant for players who lost everything
  const titleText = isVengeanceVariant ? 'Losing is a Weapon!' : 'The Escalation Engine';
  const scoreText = isVengeanceVariant ? 'VENGEANCE ×2.5 READY!' : (score + ' DAMAGE');
  const finalMsg = isVengeanceVariant 
    ? 'Your losses charged Sacrifice! Your next win gets ×2.5 damage. That\'s the engine — losing makes you stronger.'
    : 'This is where you\'re heading. Every trick makes the next one stronger.';

  popup.innerHTML = `
    <div class="aha-flash"></div>
    <div class="aha-icon">⚡</div>
    <div class="aha-title">${titleText}</div>
    <div class="aha-score">${scoreText}</div>
    ...
    <div class="aha-final-msg" id="aha-final">${finalMsg}</div>
...
// REC6-v8: Danger skull rating (1-5 skulls based on danger + act)
function getDangerSkulls(node) {
  const combatTypes = ['encounter', 'mini_boss', 'elite', 'boss', 'gauntlet'];
  if (!combatTypes.includes(node.type)) return '';
  const baseDanger = getNodeDanger(node);
  const actBonus = Math.max(0, (G.act || 1) - 1);
  const totalDanger = Math.min(5, baseDanger + actBonus);
  if (totalDanger <= 0) return '';
  const skulls = '💀'.repeat(totalDanger);
  const skullColor = totalDanger >= 4 ? '#ef4444' : totalDanger >= 3 ? '#f97316' : totalDanger >= 2 ? '#facc15' : '#4ade80';
  return `<span class="danger-skulls" style="color:${skullColor}" title="Danger: ${totalDanger}/5">${skulls}</span>`;
}

// REC6-v8: Path narrative — flavour text shown between encounters on the map
const PATH_NARRATIVES = {
  1: [
    'The ink grows thicker. Something ancient stirs ahead.',
    'Embers flicker at the edge of sight. You press deeper.',
    'The air hums with forgotten power. Your cards pulse in response.',
    'Strange marks on the walls glow faintly. A warning? Or an invitation?',
  ],
  2: [
    'The contested lands shift and churn. Reality bends around you.',
    'War drums echo from the mist. The throne is closer now.',
    'Your modifiers tremble — the enemy knows your build is growing.',
    'Ink cascades from the sky. The Monarch watches.',
  ],
  3: [
    'The Monarch\'s domain breathes with dark energy. This is the endgame.',
    'Your deck pulses with accumulated power. Every card is a weapon now.',
    'The final path opens. The throne room awaits beyond the last gate.',
    'The escalation engine roars to full power. There is no turning back.',
  ],
};
...
function showMap() {
  ...
  // REC6-v8: Path narrative — show atmospheric flavour text on the map
  let mapNarrative = document.getElementById('map-narrative');
  if (!mapNarrative) {
    mapNarrative = document.createElement('div');
    mapNarrative.id = 'map-narrative';
    mapNarrative.className = 'map-narrative';
    const mapSubheader = document.querySelector('.map-subheader');
    if (mapSubheader) mapSubheader.after(mapNarrative);
  }
  const narratives = PATH_NARRATIVES[G.act] || PATH_NARRATIVES[1];
  const completedCount = G.mapNodes.filter(n => n.completed).length;
  const narrativeIdx = completedCount % narratives.length;
  mapNarrative.textContent = narratives[narrativeIdx];
  ...
  sortedRows.forEach(rKey => {
    ...
    rowNodes.forEach(node => {
      ...
      // REC6-v8: Mark scouted nodes
      if (node._scouted) el.dataset.scouted = 'true';
...
const ENCOUNTER_RULES = {
  ...
  suit_variety: {
    ...
    color: '#e06040',
    counter: 'Keep a diverse hand. Stars cards count as any suit — save one as insurance. Avoid mono-suit builds against this enemy.',
  ...
  last_trick_only: {
    ...
    color: '#aa88cc',
    counter: 'Save your strongest card for the final trick each round. Use weaker cards early to build streaks and modifiers, then cash in on the last play.',
  ...
  descending_ranks: {
    ...
    color: '#6688cc',
    counter: 'Play your highest-rank cards first before they decay. Aces and face cards lose the most value over time — use them early.',
    onTrickStart: () => {
      if (G.trickNum > 1) {
  ...
  mirror_rows: {
    ...
    color: '#cc88aa',
    counter: 'Since every trick is a Clash, rank is everything. Pick the row whose bonus matters most and stack your highest-rank cards there. Clashes mean suit matching is guaranteed.',
  ...
  no_repeat_row: {
    ...
    color: '#88aa66',
    counter: 'Alternate between rows strategically. Plan two tricks ahead — if you need Crown, play Heart first to keep the path open. Three available rows means you always have two options.',
  ...
  escalating_cost: {
    ...
    color: '#ddaa44',
    counter: 'Stockpile Ink before this encounter. End fights quickly — every extra trick costs resources. Ink-generating relics and events are especially valuable.',
    onTrickStart: () => {
      const cost = 2;
      if (G.ink >= cost) {
        G.ink -= cost;
  ...
  countdown: {
    ...
    color: '#ff6666',
    counter: 'Maximize damage per trick — use Crown row for ×1.5 mult and chase streaks. Burn and Volatile keywords help deliver burst damage before the clock expires.',
  ...
  high_card_only: {
    ...
    color: '#cc9944',
    counter: 'Sacrifice low-rank cards as intentional losses to charge Vengeance, then unleash with your 8+ rank cards. Forge upgrades that boost rank are especially valuable.',
  ...
  suit_rotation: {
    ...
    color: '#44ccaa',
    counter: 'Track the rotation and save matching suits for the right moment. Stars cards always match — they are your ace in the hole. ×2 damage makes even weak matching cards powerful.',
    init: () => {
      const suits = getAvailableBattleSuits();
  ...
  page_tear: {
    ...
    color: '#cc7744',
    counter: 'Play your best card on trick 1 before it gets scratched. Alternatively, spread your power across multiple medium cards rather than relying on one ace.',
    onTrickStart: () => {
  ...
  rising_ranks: {
    ...
    color: '#55aacc',
    counter: 'End the fight quickly — the longer it goes, the stronger the enemy gets. Front-load your damage and use Crown row multipliers. Burn keyword helps with burst.',
    _roundCount: 0,
  ...
  blot_spread: {
    ...
    color: '#8855aa',
    counter: 'Always match the enemy suit if possible. Stars cards are safe plays. Build a suit-diverse hand before this encounter so you can always match.',
...
function startEncounter(enemy) {
  ...
  // REC1-v8: Start guided walkthrough for first encounter
  startGuidedWalkthrough();

  // REC2-v8: Apply stage-based visual collapse class
  const bs2 = document.getElementById('battle-screen');
  if (bs2) {
    bs2.classList.remove('vis-stage-1', 'vis-stage-2', 'vis-stage-3', 'vis-stage-4');
    bs2.classList.add('vis-stage-' + getOnboardingStage());
  }
...
function startTrick() {
  ...
  G._lastVengeanceApplied = false; // REC7-v8: Track for explanation bar
  G._lastCrownSurge = false; // REC7-v8: Track for explanation bar
  clearExplanationBar(); // REC7-v8: Clear persistent bar on new trick
  setBattleFocus('hand'); // TIER1-REC1v3: Focus mode — hand prominent

  // REC1-v8: Advance guided walkthrough
  if (G._guidedWalkthrough) {
    G._guidedTrickNum++;
    G._walkthroughShown = false;
    clearWalkthroughHighlights();
  }

  // REC3-v8: Guaranteed Aha Moment — trigger on first win in first encounter (or encounter 2 fallback)
  if (isAhaMomentEligible() && !G._ahaTriggered && G._guidedTrickNum <= 1) {
    // Seed spectacle on first trick if eligible — but DON'T require prior wins
    // We'll check on resolution if the player wins
  }

  // TIER1-REC2v6: Check for Aha Moment spectacle trick (LEGACY — replaced by REC3-v8)
  // Kept but modified: now triggers on any trick in first encounter when player has won at least 1
  if (isAhaMomentEligible() && !G._ahaTriggered && G.tricksWon >= 1 && G.encountersWon === 0) {
    seedSpectacleTrick();
    G._ahaTriggered = true;
    ...
    // Show walkthrough guidance after a brief delay
    if (G._guidedWalkthrough) setTimeout(() => showWalkthroughGuidance(), 500);
    return;
  }
  ...
  if (G.playerLeads) {
    ...
    enableRowSelection(true);
    updateBattleUI();
    // REC1-v8: Show walkthrough guidance
    if (G._guidedWalkthrough && !G._walkthroughShown) setTimeout(() => showWalkthroughGuidance(), 400);
  } else {
    ...
    G.phase = 'player_choose';
    enableRowSelection(true);
    // REC1-v8: Show walkthrough guidance after enemy plays
    if (G._guidedWalkthrough && !G._walkthroughShown) setTimeout(() => showWalkthroughGuidance(), 400);
  }
}
...
function playCard() {
  ...
  // REC1-v8: Clear walkthrough highlights when card is played
  clearWalkthroughHighlights();
  ...
  if (outcome.wins) {
    ...
    if (outcome.scoreResult.vengeanceApplied) {
      G.sacrificeCharge = 0;
      G._lastVengeanceApplied = true; // REC7-v8: Track for explanation bar
    ...
    if (outcome.scoreResult.crownSurgeApplied) G._lastCrownSurge = true; // REC7-v8
    ...
    // REC3-v8: Guaranteed Aha Moment — if no spectacle trick but this is the first win
    // AND aha hasn't triggered yet, show aha with the actual score  
    if (!pCard._isSpectacleCard && isAhaMomentEligible() && !G._ahaTriggered && finalDmg >= 1) {
      G._ahaTriggered = true;
      const ahaScore = Math.max(finalDmg, 30); // Show at least 30 for aha
      setTimeout(() => showAhaPopup(ahaScore), 3400);
      markAhaDone();
    }
    
    // REC3-v8: Track if player ever won in encounter 1
    if (G.encountersWon === 0) G._enc1AllLosses = false;
    
    // REC1-v8: Show post-trick walkthrough guidance
    if (G._guidedTrickNum >= 1 && G._guidedTrickNum <= 3) showPostTrickGuidance(true);
    ...
    // REC1-v8: Show post-trick walkthrough guidance on loss
    if (G._guidedTrickNum >= 1 && G._guidedTrickNum <= 3) showPostTrickGuidance(false);
    
    // REC3-v8: If player lost a trick in encounter 2 with aha still eligible,
    // and has Vengeance ready, show modified aha about the sacrifice system
    if (isAhaMomentEligible() && !G._ahaTriggered && G.encountersWon >= 1 && G.sacrificeCharge >= 3) {
      G._ahaTriggered = true;
      markAhaDone();
      setTimeout(() => {
        showAhaPopup(0, true); // vengeance variant
      }, 1500);
    }
  }
...
function showTrickResult(won, score, card) {
  ...
  // REC7-v8: Show persistent trick explanation bar (replaces transient resolution narrative)
  if (G._lastResolutionNarrative) {
    showPersistentExplanationBar(won, G._lastResolutionNarrative, G._lastLossReason || '', score);
...
// REC7-v8: PERSISTENT TRICK EXPLANATION BAR
// Single bar, fixed below enemy area, persists until next trick starts.
// Consolidates: trick result, loss reason, resolution narrative, loss effects.
function showPersistentExplanationBar(won, narrative, lossReason, score) {
  const bar = document.getElementById('trick-explanation-bar');
  const mainEl = document.getElementById('teb-main');
  const detailsEl = document.getElementById('teb-details');
  if (!bar || !mainEl) return;

  // Determine bar type and color
  let barClass = 'teb-win';
  if (!won) {
    if (lossReason && lossReason.includes('Off-suit')) barClass = 'teb-offsuit';
    else if (lossReason && lossReason.includes('Cursed')) barClass = 'teb-cursed';
    else barClass = 'teb-outranked';
  }

  bar.className = 'trick-explanation-bar visible ' + barClass;

  // Build main line — compact summary
  if (won) {
    const streakRow = G.selectedRow || 'crown';
    const streak = G.rowStreaks[streakRow] || 0;
    const streakText = streak >= 2 ? ` | 🔗 Streak ×${streak}` : '';
    const vengeText = G._lastVengeanceApplied ? ' | 💀 VENGEANCE ×2.5' : '';
    const crownText = G._lastCrownSurge ? ' | 👑 Crown Surge ×2' : '';
    mainEl.innerHTML = `<span class="teb-icon">✅</span> <span class="teb-text">Won +${score} dmg on ${streakRow.charAt(0).toUpperCase() + streakRow.slice(1)}${streakText}${vengeText}${crownText}</span>`;
  } else {
    // Parse loss reason into compact format
    const sacCharge = G.sacrificeCharge || 0;
    const sacText = sacCharge >= 3 ? '⚡ VENGEANCE READY' : `🗡️ Sacrifice ${sacCharge}/3`;
    mainEl.innerHTML = `<span class="teb-icon">❌</span> <span class="teb-text">${lossReason || 'Lost'}</span> <span class="teb-sep">|</span> <span class="teb-text">${sacText}</span>`;
  }

  // Details (tap to toggle)
  detailsEl.textContent = narrative;
...
// REC7-v8: Clear the persistent bar when a new trick starts
function clearExplanationBar() {
  const bar = document.getElementById('trick-explanation-bar');
  if (bar) {
    bar.className = 'trick-explanation-bar';
    bar.onclick = null;
  }
}
...
function showRest() {
  ...
  const choices = [
    ...
    // REC6-v8: Scout option — spend Ink to reveal next 2 nodes' full details
    {
      label: '🔭 Scout Ahead',
      desc: `Spend 15 Ink to reveal the next 2 combat nodes' enemies, rules, and HP`,
      icon: '🔭',
      category: 'power',
      disabled: G.ink < 15,
      disabledReason: 'Not enough Ink (need 15)',
      fn: () => {
        G.ink -= 15;
        // Reveal next 2 unrevealed combat nodes
        const currentNodeObj = G.mapNodes.find(n => n.id === G.currentNode);
        const toReveal = [];
        if (currentNodeObj) {
          const queue = [...currentNodeObj.children];
          const visited = new Set();
          while (queue.length > 0 && toReveal.length < 2) {
            const nid = queue.shift();
            if (visited.has(nid)) continue;
            visited.add(nid);
            const node = G.mapNodes.find(n => n.id === nid);
        ...
        const revealCount = toReveal.length;
        spawnKeywordPopup(`🔭 Scouted ${revealCount} node${revealCount !== 1 ? 's' : ''} ahead!`, 40);
        showMap();
      }
    },
  ];
...
function showForge() {
  ...
  if (!META.tooltipsSeen?.forge_comparison) {
    ...
    compPanel.innerHTML = `
      ...
      <div class="forge-compare-footer">★ Best markers below show the upgrade path that best fits your current build and archetype.</div>
    `;
...
function toggleModStripExpand() {
  G.modStripExpanded = !G.modStripExpanded;
  renderModifierStrip();
}
...
function renderModifierStrip() {
  ...
  if (G.modifiers.length === 0) {
    ...
    strip.innerHTML = '<div class="power-level-display power-empty" onclick="toggleModStripExpand()"><span class="pl-icon">⚡</span><span class="pl-mult">×1.0</span><span class="pl-label-text">Power Level</span></div>';
    G._prevPowerLevel = 1.0;
    return;
  }
  strip.classList.add('has-mods');

  // REC4-v8: Calculate contextual Power Level
  // If a card is selected, show multiplier for that card's suit specifically
  const selectedSuit = G.selectedCard ? G.selectedCard.suit : null;
  ...
  G.modifiers.forEach(m => {
    const isUniversal = m.suit === null || m.suit === 'stars' || (typeof hasArchetypeMilestone === 'function' && hasArchetypeMilestone('all_universal'));
    if (m.type === 'mult') {
      totalMult += m.value;
      if (selectedSuit && (m.suit === selectedSuit || isUniversal)) activeMult += m.value;
    } else {
      totalChips += m.value;
      if (selectedSuit && (m.suit === selectedSuit || isUniversal)) activeChips += m.value;
    }
  });
  
  // Use active (card-specific) or total depending on context
  const displayMult = selectedSuit ? activeMult : totalMult;
  const displayChips = selectedSuit ? activeChips : totalChips;
  
  // REC4-v8: Color-coded thresholds
  let plColor, plTier;
  if (displayMult >= 16) { plColor = '#ff4444'; plTier = 'INFERNO'; }
  else if (displayMult >= 8) { plColor = '#ff8c00'; plTier = 'BLAZING'; }
  else if (displayMult >= 4) { plColor = '#ffd700'; plTier = 'FLAME'; }
  else if (displayMult >= 2) { plColor = '#66ff66'; plTier = 'SPARK'; }
  else { plColor = '#cccccc'; plTier = 'BASE'; }
  
  // REC4-v8: Detect modifier gain for animation
  const prevPower = G._prevPowerLevel || 1.0;
  const isNewMod = displayMult > prevPower + 0.05;
  G._prevPowerLevel = displayMult;
  
  // Build the power level display
  const plDiv = document.createElement('div');
  plDiv.className = 'power-level-display' + (G.modStripExpanded ? ' expanded' : '') + (isNewMod ? ' pl-pulse' : '');
  plDiv.onclick = () => { G.modStripExpanded = !G.modStripExpanded; renderModifierStrip(); };
  
  const suitContext = selectedSuit ? `<span class="pl-suit-ctx" style="color:${getComputedSuitColor(selectedSuit)}">${SUIT_SYMBOLS[selectedSuit]}</span>` : '';
  
  plDiv.innerHTML = `
    <div class="pl-main">
      <span class="pl-icon">⚡</span>
      <span class="pl-mult" style="color:${plColor}">×${displayMult.toFixed(1)}</span>
      ${suitContext}
      <span class="pl-tier" style="color:${plColor}">${plTier}</span>
      <span class="pl-chips-badge">+${Math.floor(displayChips)}</span>
      <span class="pl-expand-btn">${G.modStripExpanded ? '▲' : '▼'}</span>
    </div>
    <div class="pl-sub">${G.modifiers.length} mods${selectedSuit ? ' · showing ' + selectedSuit : ''}</div>
  `;
  strip.appendChild(plDiv);

  // TIER2-9: Build Identity Display
  const lead = getLeadArchetype();
  if (lead && G.modStripExpanded) {
    const arch = ARCHETYPE_DATA[lead.key];
    const archDiv = document.createElement('div');
    archDiv.className = 'build-arch-badge';
    archDiv.innerHTML = `<span style="color:${arch.color}">${arch.icon} ${arch.name}</span>`;
    strip.appendChild(archDiv);
  }
  ...
  if (G.modStripExpanded) {
    ...
    const suitBar = document.createElement('div');
    suitBar.className = 'build-suit-bar';
    availSuits.forEach(s => {
      const pct = Math.max(2, (suitCounts[s] / totalSuitMods) * 100);
      if (suitCounts[s] > 0) {
        const seg = document.createElement('div');
        seg.className = 'bsb-seg';
        seg.style.width = pct + '%';
        seg.style.background = getComputedSuitColor(s);
        seg.title = (SUIT_SYMBOLS[s] || s) + ' ' + suitCounts[s];
        suitBar.appendChild(seg);
      }
    });
    strip.appendChild(suitBar);
  }
  ...
  if (G.modStripExpanded) {
    ...
    sorted.forEach((m, i) => {
      ...
      const isUniversal = m.suit === null || m.suit === 'stars';
      const applies = selectedSuit && (m.suit === selectedSuit || isUniversal);
      const pip = document.createElement('div');
      pip.className = 'mod-pip ' + (m.tier || 'spark');
      if (isDisabled) pip.classList.add('disabled');
      if (isNewMod && globalIdx >= G.modifiers.length - 1) pip.classList.add('mod-new');
      if (m._decisionId) pip.classList.add('decision-mod');
      if (selectedSuit && applies) pip.classList.add('mod-active');
      else if (selectedSuit && !applies) pip.classList.add('mod-inactive');
    ...
    // Modifier contribution summary when card is selected
    if (selectedSuit) {
      const summaryEl = document.createElement('div');
      summaryEl.className = 'mod-contribution-summary';
      summaryEl.innerHTML = `Active for ${SUIT_SYMBOLS[selectedSuit] || '?'}: <span class="mod-contrib-chips">+${activeChips.toFixed(1)} chips</span> · <span class="mod-contrib-mult">×${activeMult.toFixed(2)} mult</span>`;
    ...
    if (decMods.length > 0) {
      const decisionHtml = document.createElement('div');
      decisionHtml.className = 'build-decision-mods';
      decMods.forEach(m => {
        const badge = document.createElement('span');
        badge.className = 'build-decision-badge';
        badge.title = m._decisionDesc || '';
        badge.textContent = m.name;
        decisionHtml.appendChild(badge);
      });
      strip.appendChild(decisionHtml);
    }
  }
}
...
function renderHand() {
  ...
  G.hand.forEach(card => {
    ...
    // Build keyword display - show icons + text for visual identity
    // REC10v7: Keyword iconography — emoji icons replace plain text
    let kwHtml = '';
    if (card.keywords.length > 0) {
      const isHighRarity = card.rarity === 'rare' || card.rarity === 'epic' || card.rarity === 'legendary';
      kwHtml = '<div class="card-keywords' + (isHighRarity ? ' kw-prominent' : '') + '">' + card.keywords.map(kw => {
        const icon = (window.KEYWORD_ICONS && window.KEYWORD_ICONS[kw]) || '';
        return `<span class="card-kw-tag" data-kw="${kw}">${icon ? '<span class="kw-icon">' + icon + '</span>' : ''}${kw}</span>`;
      }).join('') + '</div>';
    }
    ...
    // REC5-v8: Double-click/double-tap quick-play — select card + auto-pick best row + play
    let _lastClickTime = 0;
    el.addEventListener('click', (e) => {
      const now = Date.now();
      if (now - _lastClickTime < 400 && G.selectedCard?.id === card.id && G.phase === 'player_choose') {
        // Double-click detected — auto-pick best row and play
        if (!G.selectedRow) {
          const bestRow = getBestRowForCard(card);
          if (bestRow) {
            G.selectedRow = bestRow;
            updateRowHighlight();
          }
        }
        if (G.selectedRow) playCard();
      }
      _lastClickTime = now;
    });

    // REC5-v8: Desktop drag-to-row — drag card onto a row lane
    el.setAttribute('draggable', 'true');
    el.addEventListener('dragstart', (e) => {
      if (G.phase !== 'player_choose') { e.preventDefault(); return; }
      selectCard(card.id);
      e.dataTransfer.setData('text/plain', card.id);
      e.dataTransfer.effectAllowed = 'move';
      el.classList.add('dragging');
      // Highlight droppable row lanes
      document.querySelectorAll('.row-lane.selectable').forEach(r => r.classList.add('drop-target'));
    });
    el.addEventListener('dragend', () => {
      el.classList.remove('dragging');
      document.querySelectorAll('.row-lane').forEach(r => r.classList.remove('drop-target', 'drop-hover'));
    });

    // REC10v7: Desktop hover tooltip — keyword descriptions + per-row damage estimates
    let hoverTimer = null;
    el.addEventListener('mouseenter', () => {
      hoverTimer = setTimeout(() => {
        showCardHoverTooltip(card, el);
      }, 350);
    });
    ...
    let _lastTapTime = 0; // REC5-v8: Double-tap tracking
    ...
    el.addEventListener('touchend', (e) => {
      ...
      if (!isSmallScreen && diffY > 50 && diffX < 80 && G.selectedCard?.id === card.id && G.selectedRow) {
        ...
        e.preventDefault();
        // REC5-v8: Double-tap quick-play on mobile
        const now = Date.now();
        if (now - _lastTapTime < 400 && G.selectedCard?.id === card.id && G.phase === 'player_choose') {
          if (!G.selectedRow) {
            const bestRow = getBestRowForCard(card);
            if (bestRow) { G.selectedRow = bestRow; updateRowHighlight(); }
          }
          if (G.selectedRow) playCard();
        } else {
          selectCard(card.id);
...
function showCardInspect(card) {
  ...
  detail.innerHTML = `
    ...
    ${card.keywords.length ? '<div class="inspect-keywords">' + card.keywords.map(kw => {
      const icon = (window.KEYWORD_ICONS && window.KEYWORD_ICONS[kw]) || '';
      return `<div class="inspect-kw"><span class="inspect-kw-name">${icon ? icon + ' ' : ''}${kw}</span><span class="inspect-kw-desc">${KEYWORD_DESCRIPTIONS[kw] || ''}</span></div>`;
...
// REC10v7: Desktop hover tooltip — shows keyword descriptions, suit reward, per-row damage
function showCardHoverTooltip(card, anchorEl) {
  hideCardHoverTooltip();
  const tooltip = document.createElement('div');
  tooltip.id = 'card-hover-tooltip';
  tooltip.className = 'card-hover-tooltip';

  const suitRewards = { hearts: '♥ +mult', diamonds: '♦ +chips', clubs: '♣ direct dmg', spades: '♠ reveal card', stars: '⭐ universal mod' };
  const suitColor = SUIT_COLORS[card.suit] || 'var(--text-primary)';

  // Keyword descriptions
  let kwSection = '';
  if (card.keywords.length > 0) {
    kwSection = '<div class="hover-kw-section">' + card.keywords.map(kw => {
      const icon = (window.KEYWORD_ICONS && window.KEYWORD_ICONS[kw]) || '';
      return `<div class="hover-kw-row"><span class="hover-kw-name">${icon ? icon + ' ' : ''}${kw}</span><span class="hover-kw-desc">${KEYWORD_DESCRIPTIONS[kw] || ''}</span></div>`;
    }).join('') + '</div>';
  }

  // Per-row damage estimates
  let rowEstimates = '';
  if (G.phase === 'player_choose' && G.enemy) {
    const rows = [
      { key: 'crown', label: '👑 Crown', color: 'var(--crown-row)' },
      { key: 'heart', label: '❤️ Heart', color: 'var(--heart-row)' },
      { key: 'foundation', label: '🛡️ Shield', color: 'var(--foundation-row)' }
    ];
    rowEstimates = '<div class="hover-row-estimates">';
    rows.forEach(r => {
      const est = calculateScore(card, r.key);
      rowEstimates += `<div class="hover-row-est"><span style="color:${r.color}">${r.label}</span><span class="hover-est-val">~${est}</span></div>`;
    });
    rowEstimates += '</div>';
  }
  ...
  tooltip.innerHTML = `
    <div class="hover-header">
      <span class="hover-card-name" style="color:${suitColor}">${RANK_NAMES[card.rank]}${SUIT_SYMBOLS[card.suit]} ${card.name || ''}</span>
      <span class="hover-rarity hover-rarity-${card.rarity || 'common'}">${card.rarity || 'common'}</span>
    </div>
    <div class="hover-stats">${card.baseChips}♦ chips • ${card.baseMult}× mult</div>
    <div class="hover-suit-reward" style="color:${suitColor}">${suitRewards[card.suit] || ''}</div>
    ${kwSection}
    ${rowEstimates}
  `;
...
function hideCardHoverTooltip() {
  const existing = document.getElementById('card-hover-tooltip');
  if (existing) existing.remove();
}
...
function enableRowSelection(enable) {
  ...
  document.querySelectorAll('.row-lane').forEach(el => {
    ...
    if (enable && isAvailable && !isRuleBlocked) {
      ...
      // REC5-v8: Drop target for drag-to-row
      el.addEventListener('dragover', _rowDragOver);
      el.addEventListener('dragenter', _rowDragEnter);
      el.addEventListener('dragleave', _rowDragLeave);
      el.addEventListener('drop', _rowDrop);
    } else {
      el.classList.remove('selectable', 'selected');
      el.onclick = null;
      el.removeEventListener('dragover', _rowDragOver);
      el.removeEventListener('dragenter', _rowDragEnter);
      el.removeEventListener('dragleave', _rowDragLeave);
      el.removeEventListener('drop', _rowDrop);
      if (!isAvailable) {
...
// REC5-v8: Drag-to-row event handlers
function _rowDragOver(e) { e.preventDefault(); e.dataTransfer.dropEffect = 'move'; }
function _rowDragEnter(e) { e.preventDefault(); this.classList.add('drop-hover'); }
function _rowDragLeave(e) { this.classList.remove('drop-hover'); }
function _rowDrop(e) {
  e.preventDefault();
  this.classList.remove('drop-hover');
  document.querySelectorAll('.row-lane').forEach(r => r.classList.remove('drop-target', 'drop-hover'));
  const row = this.dataset.row;
  if (row && G.selectedCard && G.phase === 'player_choose') {
    G.selectedRow = row;
    updateRowHighlight();
    // Auto-play immediately on drag-drop
    setTimeout(() => playCard(), 50);
  }
}

// REC5-v8: Pick the best row for a card based on streak status and strategy
function getBestRowForCard(card) {
  const available = getAvailableRows();
  if (available.length === 1) return available[0];
  
  // Priority: row closest to surge (streak 3), then crown for damage, then heart, then shield
  let best = null, bestScore = -1;
  available.forEach(row => {
    let score = 0;
    const streak = G.rowStreaks[row] || 0;
    if (streak === 3) score += 10; // About to surge
    else if (streak >= 2) score += 5;
    if (row === 'crown') score += 3; // Crown for damage
    else if (row === 'heart') score += 2;
    else score += 1;
...
function getComputedSuitColor(suit) {
  // REC8-v8: Use Okabe-Ito colorblind-safe palette in high-contrast mode
  if (document.body.classList.contains('high-contrast')) {
    return { hearts: '#D55E00', diamonds: '#0072B2', clubs: '#009E73', spades: '#E69F00', stars: '#F0E442' }[suit] || '#888';
  }
  ...
  Object.entries(categories).forEach(([catKey, cat]) => {
    ...
    cat.keys.forEach(key => {
      ...
      html += `<div class="prog-item rule-item ${found ? 'unlocked' : 'locked'}">
        ...
        ${found && rule.counter ? `<div class="rule-counter"><span class="rule-counter-label">💡 Strategy:</span> ${rule.counter}</div>` : ''}
...
document.addEventListener('keydown', (e) => {
  if (G.phase === 'player_choose') {
    // REC8-v8: Arrow key navigation for cards and rows
    if (e.key === 'ArrowLeft' || e.key === 'ArrowRight') {
      e.preventDefault();
      const cards = G.hand;
      if (cards.length === 0) return;
      const currentIdx = G.selectedCard ? cards.findIndex(c => c.id === G.selectedCard.id) : -1;
      let nextIdx;
      if (e.key === 'ArrowRight') {
      ...
      selectCard(cards[nextIdx].id);
      // Focus the visual card element for screen readers
      const cardEls = document.querySelectorAll('.hand-card');
      if (cardEls[nextIdx]) cardEls[nextIdx].focus();
    }
    if (e.key === 'ArrowUp' || e.key === 'ArrowDown') {
      e.preventDefault();
      const available = getAvailableRows();
      if (available.length === 0) return;
      const currentIdx = G.selectedRow ? available.indexOf(G.selectedRow) : -1;
      let nextIdx;
      if (e.key === 'ArrowDown') {
        nextIdx = currentIdx < available.length - 1 ? currentIdx + 1 : 0;
      } else {
        nextIdx = currentIdx > 0 ? currentIdx - 1 : available.length - 1;
      }
      selectRow(available[nextIdx]);
      // Announce to screen reader
      const rowNames = { crown: 'Crown (damage)', heart: 'Heart (chips)', foundation: 'Shield (defend)' };
      const announcement = rowNames[available[nextIdx]] || available[nextIdx];
      announceToScreenReader('Row: ' + announcement);
    }
...
// REC8-v8: Screen reader announcements
