...
function showForge() {
  ...
  flameMods.forEach((mod, i) => {
    ...
    // TIER2-10 + TIER3-13: Stable conditional — assigned per modifier using persistent _forgeCondKey
    ...
    // Falls back to hash-based seed. Uses modifier id/creation-order for stability.
    if (!mod._forgeCondKey) {
      if (mod._seededCondIdx !== undefined && mod._seededCondIdx !== null) {
        ...
        let hash = 0;
        // Use id if available, otherwise name+suit+value for stable seed
...
const CONTEXTUAL_TIPS = {
  ...
  bid: () => ({
    title: '🎲 Bid Format',
    body: 'Predict how many tricks you\'ll win before the fight. Accurate prediction earns bonus Ink. The closer your bid, the bigger the reward!',
    icon: '🎲',
  }),
};
