...
function generateNodePreview(node) {
  switch (node.type) {
    ...
    case 'rest':
      return { line1: '🔥 Rest Site', line2: '6 options: Heal, Smith, Remove...', tier: 'safe' };
    case 'shop':
      return { line1: '🏪 Card Shop', line2: 'Buy cards, upgrades, healing', tier: 'safe' };
    case 'treasure':
    ...
    case 'gauntlet':
      return { line1: '🏟️ Gauntlet', line2: '5 quick fights · Bonus Ink', tier: 'hard' };
    case 'puzzle':
      return { line1: '🧩 Puzzle', line2: 'Win exactly 2 of 4 tricks', tier: 'medium' };
    case 'bid':
      return { line1: '🎲 Bid Challenge', line2: 'Predict your wins for bonus', tier: 'medium' };
    default:
...
function showMap() {
  ...
  sortedRows.forEach(rKey => {
    ...
    rowNodes.forEach(node => {
      ...
      // TIER2-7: Enhanced preview tooltip
      const preview = node._preview;
      const danger = node._danger || 0;
      if (preview) {
        const dangerStars = danger >= 3 ? '🔴🔴🔴' : danger >= 2 ? '🟠🟠' : danger >= 1 ? '🟡' : '🟢';
        const dangerLabel = danger >= 3 ? 'Extreme' : danger >= 2 ? 'Dangerous' : danger >= 1 ? 'Standard' : 'Safe';
        el.innerHTML = `${node.icon}<div class="node-preview-tooltip"><div class="npt-title">${preview.line1}</div><div class="npt-desc">${preview.line2}</div><div class="npt-danger">${dangerStars} ${dangerLabel}</div></div>`;
...
function getEncounterRule(enemy) {
  ...
  if (enemy._isMiniBoss) return null;
...
function resolveTrick(pCard, row, eCard) {
  ...
  const suitMatches = pCard.suit === eCard.suit || pCard.suit === 'stars' || eCard.suit === 'stars';
  ...
  const enemyFollowedSuit = (eCard.suit === pCard.suit) || (eCard.suit === 'stars') || (pCard.suit === 'stars');
...
function showScoreBreakdown(data) {
  ...
  overlay.classList.remove('threshold-50', 'threshold-100', 'threshold-250', 'threshold-500', 'inferno-glow');
...
function showForge() {
  ...
  flameMods.forEach((mod, i) => {
    ...
    // TIER2-10: Seeded conditional — assigned per modifier, not randomized on re-render
    ...
    // Use a deterministic seed based on modifier name + suit + tier (NOT index, which shifts)
    if (!mod._seededCondIdx && mod._seededCondIdx !== 0) {
      let hash = 0;
      const seedStr = (mod.name || '') + (mod.suit || '') + (mod.tier || '') + (mod.type || '');
      for (let ci = 0; ci < seedStr.length; ci++) {
        hash = ((hash << 5) - hash) + seedStr.charCodeAt(ci);
        hash |= 0;
      }
      mod._seededCondIdx = Math.abs(hash) % condEffects.length;
    }
    const randomCond = condEffects[mod._seededCondIdx];
...
// ============================================================
// TIER1-REC1v3: BATTLE SCREEN FOCUS MODE
// Three visual states with CSS transitions:
//   hand    — Hand prominent, compact rows, enemy summary only
//   resolving — Full trick animation, enemy card reveal, score breakdown
//   summary — Round summary with accumulated effects before next round
// ============================================================
