...
function showAhaPopup(score) {
  ...
  popup.innerHTML = `
    ...
    <div class="aha-title">That's the Escalation Engine!</div>
    <div class="aha-score">${score} DAMAGE</div>
    <div class="aha-desc">Build your modifiers and it only gets crazier.<br>Stack suits, chain streaks, forge upgrades — the ceiling is limitless.</div>
...
function showMap() {
  ...
  sortedRows.forEach(rKey => {
    ...
    rowNodes.forEach(node => {
      ...
      // TIER1-REC3v6: Enhanced preview tooltip with difficulty, HP, and path preview
      ...
      if (preview) {
        ...
        const pathLine = pathPreview ? `<div class="npt-path">${pathPreview}</div>` : '';
        tooltipHtml = `<div class="node-preview-tooltip"><div class="npt-title">${preview.line1}</div><div class="npt-desc">${preview.line2}</div>${hpLine}${diffLine}${pathLine}</div>`;
...
function generateRunInsights(victory) {
  ...
  if (offSuitLosses >= 3) {
    insights.push({
      icon: '🎴',
      text: `You lost ${offSuitLosses} tricks to off-suit plays. Consider <strong>building more suit coverage</strong> or using <strong>Stars cards</strong> as universal answers.`,
      type: 'warning'
    });
