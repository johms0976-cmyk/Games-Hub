...
function loadMeta() {
  try {
    ...
    if (raw) return { ...META_DEFAULTS, ...JSON.parse(raw) };
...
async function loadProgressionData() {
  try {
    ...
    console.warn('Progression data load failed, using inline fallback');
    UNLOCK_DATA = [];
    ACHIEVEMENT_DATA = [];
  }
}
...
function startGame() {
  ...
  if (stage <= 4) {
    ...
    // For stages 2-4, show intro on map before first move
    showOnboardingIntro(stage);
  }
}
...
function generateMap() {
  ...
  const nodeTypes = [
    { type: 'encounter', icon: '⚔️', label: 'Encounter', weight: 30 },
    ...
    { type: 'gauntlet', icon: '🏟️', label: 'Gauntlet', weight: 7 },
    { type: 'puzzle', icon: '🧩', label: 'Puzzle', weight: 6 },
    { type: 'bid', icon: '🎲', label: 'Bid', weight: 5 },
  ];
...
function startGauntletEncounter() {
  // Gauntlet: 5 quick enemies, 1 trick each
...
function advanceGauntletWave() {
  ...
  const miniHp = Math.floor((template.baseHp + G.act * template.hpPerAct) * 0.3);
  const enemy = {
    name: '⚡ ' + template.name,
    ...
    hand: [],
    tricksPerRound: 1,
    intent: 'attack', intentQueue: ['attack'],
    currentPhase: 0, disabledModIdx: -1,
  };

  // Generate one enemy card
  const suit = randomBattleSuit();
  const rank = Math.min(14, Math.floor(Math.random() * 10) + 2 + G.act);
  ...
  // Only reset full state on first wave
  if (G.gauntletWave === 1) {
...
function startBidEncounter() {
  // Bid: Predict how many tricks you'll win
  ...
  document.getElementById('event-text').textContent = `A spectral dealer appears. "Predict your fate — how many of ${enemy.tricksPerRound} tricks will you win? Precision is rewarded."`;
  ...
  for (let i = 0; i <= enemy.tricksPerRound; i++) {
    ...
    const riskLabel = i === 0 ? ' (Humble)' : i === enemy.tricksPerRound ? ' (Bold!)' : '';
    el.innerHTML = `<div class="choice-label">Bid: ${i} wins${riskLabel}</div><div class="choice-desc">Win exactly ${i} of ${enemy.tricksPerRound} tricks for bonus rewards</div>`;
    el.addEventListener('click', () => {
      G.bidPrediction = i;
      startEncounter(enemy);
    });
...
function checkSpecialEncounterEnd() {
  ...
  if (G.encounterFormat === 'gauntlet') {
    ...
    if (G.enemy.hp <= 0) {
      ...
      spawnKeywordPopup(`🏟️ Wave ${G.gauntletWave}/${G.gauntletMaxWaves} CLEAR!`, 30);
    } else {
...
// Map passiveKey to encounter rule key
const PASSIVE_TO_RULE = {
  suit_restrict: 'suit_variety',       // Suit Binder gets the suit variety rule
  damage_reduction: 'last_trick_only', // Fade Wraith gets last-trick-only
  win_punish: 'countdown',             // Grudge Sentinel gets countdown pressure
  armor_on_player_win: 'mirror_rows',  // Patience Weaver mirrors your row
  ink_steal: 'escalating_cost',        // Ink Leech gets escalating toll
  ember_drain: 'descending_ranks',     // Ember Moth gets descending ranks
  double_play: 'high_card_only',       // Twin Dealer gets high-card gate
  regen_armor: 'no_repeat_row',        // Ironclad Scribe gets path blocking
};

// Enemies without passiveKey get a random rule from this pool
const RANDOM_RULE_POOL = ['suit_rotation', 'descending_ranks', 'mirror_rows', 'no_repeat_row'];

function getEncounterRule(enemy) {
  if (enemy.tier !== 'standard') return null;
  if (enemy._isMiniBoss) return null; // Mini-bosses have their own wrinkles
  // Check if this enemy has a mapped encounter rule
  if (enemy.passiveKey && PASSIVE_TO_RULE[enemy.passiveKey]) {
    return ENCOUNTER_RULES[PASSIVE_TO_RULE[enemy.passiveKey]] || null;
  }
  // Enemies without passiveKey get a random rule
  if (!enemy.passiveKey) {
    const key = RANDOM_RULE_POOL[Math.floor(Math.random() * RANDOM_RULE_POOL.length)];
    return ENCOUNTER_RULES[key] || null;
  }
  return null;
}
...
function resolveTrick(pCard, row, eCard) {
  ...
  const enemyFollowedSuit = (eCard.suit === pCard.suit) || (eCard.suit === 'stars');
  const playerFollowedSuit = suitMatches; // already checks pCard.suit === eCard.suit || stars
...
function playCard() {
  ...
  if (outcome.wins) {
    ...
    // TIER1-REC5: Shieldbreaker — convert shield to bonus damage
    if (G.shieldBreakerReady && G.shield > 0) {
      const shieldDmg = G.shield;
      finalDmg += shieldDmg;
      G.shield = 0;
      G.shieldBreakerReady = false;
      collectTrickEffect(`🛡️→⚔️ Shieldbreaker: +${shieldDmg} from shield`, 'bonus-color');
    }
    ...
    if (G.enemy.passiveKey === 'damage_reduction' && G.enemy.hp > G.enemy.maxHp * 0.5) {
  ...
  setTimeout(() => {
    ...
    if (G.enemy.hp <= 0) {
      // TIER1-3: Bid encounter — check bid accuracy before victory
      if (G.encounterFormat === 'bid' && G.bidPrediction >= 0) {
        const accurate = G.bidTricksWon === G.bidPrediction;
        if (accurate) {
          const bidBonus = 40 + G.bidPrediction * 15;
          G.ink += bidBonus;
          spawnKeywordPopup(`🎲 PERFECT BID! +${bidBonus} bonus Ink!`, 30);
        } else {
          spawnKeywordPopup(`🎲 Bid: ${G.bidTricksWon}/${G.bidPrediction} — close!`, 38);
        }
        G.encounterFormat = 'standard';
      }
...
function applyRowSurge(row) {
  ...
  switch (row) {
    case 'crown':
      ...
      break;
    case 'heart':
      // TIER1-REC5: Heart Chip Doubler — ALL chip modifiers doubled for rest of encounter
      ...
      break;
    case 'foundation':
      // TIER1-REC5: Shieldbreaker — next win converts total shield into bonus damage
      const shieldSurge = Math.floor(G.maxHp * 0.5);
      G.shield += shieldSurge;
      G.shieldBreakerReady = true;
      spawnCriticalPopup('🛡️⚔️ SHIELDBREAKER! Shield→Damage on next win!', 28);
      break;
  }
}
...
function showScoreBreakdown(data) {
  ...
  allSteps.forEach((step, i) => {
    ...
    setTimeout(() => {
      ...
      AudioEngine.breakdownStep(i, allSteps.length);
...
function showTrickResult(won, score, card) {
  ...
  if (won) AudioEngine.trickWin(G.consecutiveWins);
  else AudioEngine.trickLose();
  if (won && score > 0) {
...
function showRest() {
  ...
  const choices = [
    ...
    },
    {
      ...
      desc: 'Choose a card to receive a random keyword',
      icon: '🔑',
      category: 'cards',
      needsCardSelect: true,
      fn: (card) => {
        ...
        if (available.length > 0) {
          ...
          card.name = '✦ ' + card.name.replace(/^✦ /, '');
          spawnKeywordPopup(`Enchanted: ${kw}!`, 40);
        }
        showMap();
      }
    },
...
function showForge() {
  ...
  flameMods.forEach((mod, i) => {
    ...
    html += `<div class="forge-path">
      <button class="forge-upgrade-btn forge-path-a" ${G.embers < boostCost ? 'disabled' : ''} data-action="boost" data-mod-idx="${G.modifiers.indexOf(mod)}">
        🔥 Inferno (${boostCost}🔥)<br><span class="forge-path-desc">${valStr} → ${boostedVal} ${mod.type}</span>
      </button>`;

    // Path B: Change Modifier Suit
    const suitCost = Math.floor(baseCost * 0.8);
    html += `<button class="forge-upgrade-btn forge-path-b" ${G.embers < suitCost ? 'disabled' : ''} data-action="resuit" data-mod-idx="${G.modifiers.indexOf(mod)}">
        🎨 Re-suit (${suitCost}🔥)<br><span class="forge-path-desc">Change to any suit or universal</span>
      </button>`;
    ...
    const randomCond = condEffects[Math.floor(Math.random() * condEffects.length)];
    html += `<button class="forge-upgrade-btn forge-path-c" ${G.embers < condCost ? 'disabled' : ''} data-action="condition" data-mod-idx="${G.modifiers.indexOf(mod)}" data-cond-key="${randomCond.key}">
        ✨ ${randomCond.name} (${condCost}🔥)<br><span class="forge-path-desc">${randomCond.desc}</span>
      </button>
    </div>`;
...
function gameOver(victory) {
  ...
  document.getElementById('gameover-stats').innerHTML = `
    ...
    <br><span class="gameover-collection-line">🏆 ${achvCount} Achievements | 🔓 ${newUnlockCount} Unlocks | 📖 ${META.codex.length} Cards | 🔍 ${META.bestiary.length} Relics</span>
...
function updateBattleUI() {
  ...
  else if (G.encounterFormat === 'bid') trickText = `Bid:${G.bidPrediction} Won:${G.bidTricksWon} (${G.trickNum}/${G.tricksPerRound})`;
  ...
  if (G.enemy) {
    ...
    if (formatBanner) {
      if (G.encounterFormat === 'gauntlet') {
        ...
        formatBanner.textContent = `🏟️ GAUNTLET — Wave ${G.gauntletWave}/${G.gauntletMaxWaves}`;
        ...
        formatBanner.textContent = `🎲 BID ${G.bidPrediction} — Won ${G.bidTricksWon} so far`;
      } else {
      ...
      if (ruleBanner && rule) {
        ...
        let ruleText = `${rule.icon} ${rule.name}: ${rule.desc}`;
        // Show rotation suit for Prismatic Drake
        if (rule.getRequiredSuit) {
          const reqSuit = rule.getRequiredSuit();
          if (reqSuit) ruleText += ` — Current: ${SUIT_SYMBOLS[reqSuit]} ${reqSuit}`;
        }
        // Show shieldbreaker/chip doubler status
        if (G.heartChipDoubler) ruleText = '❤️🔥 CHIP DOUBLER ACTIVE · ' + ruleText;
        if (G.shieldBreakerReady) ruleText = '🛡️⚔️ SHIELDBREAKER READY · ' + ruleText;
        ruleBanner.innerHTML = ruleText;
        ...
        if (G.heartChipDoubler || G.shieldBreakerReady) {
          ruleBanner.style.display = 'block';
          let powerText = '';
          ...
          ruleBanner.style.borderColor = G.heartChipDoubler ? '#e63946' : '#55aa88';
          ruleBanner.style.color = G.heartChipDoubler ? '#e63946' : '#55aa88';
        } else {
...
// ============================================================
// FEATURE 3: MODIFIER STRIP RENDERING
// ============================================================
let _prevModCount = 0;
...
function renderModifierStrip() {
  ...
  // Power Level summary indicator
  const plDiv = document.createElement('div');
  plDiv.className = 'power-level-indicator' + (G.modStripExpanded ? ' expanded' : '') + (isNew ? ' pl-flash' : '');
  plDiv.onclick = () => { G.modStripExpanded = !G.modStripExpanded; renderModifierStrip(); };
  plDiv.innerHTML = `
    <span class="pl-label" style="color:${tierColor}">⚡ ${tierLabel}</span>
    <span class="pl-value" style="color:${tierColor}">${powerLevel}</span>
    <span class="pl-detail">${G.modifiers.length} mods • ${totalMult.toFixed(1)}× • +${totalChips} chips</span>
  ...
  if (G.modStripExpanded) {
    ...
    sorted.forEach((m, i) => {
      ...
      const valStr = m.type === 'mult' ? (typeof m.value === 'number' ? m.value.toFixed(2) : m.value) + '×' : '+' + m.value;
      const suitIcon = m.suit ? (SUIT_SYMBOLS[m.suit] || m.suit[0]) : '✦';
      pip.innerHTML = `${suitIcon} ${valStr}<div class="mod-pip-tooltip">${m.name}: ${valStr} ${m.type}${m.suit ? ' (' + m.suit + ')' : ' (all)'}${isDisabled ? ' — DISABLED' : ''}</div>`;
      modsContainer.appendChild(pip);
    });
